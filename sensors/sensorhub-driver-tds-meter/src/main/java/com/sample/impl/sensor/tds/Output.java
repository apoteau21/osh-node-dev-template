    ...
    void doInit() {
        ...
        dataStruct = sweFactory.createRecord()
                ...
                .addField("Timestamp", sweFactory.createTime()
                        ...
                        .build())
                .addField("Millivolts", sweFactory.createText()
                        .label("Milivolts")
                        .description("The level of millivolts (mV) in the water.")
                        .build())
                .addField("Parts Per Million", sweFactory.createText()
                        .label("Parts Per Million")
                        .description("The level of parts per million (ppm) in the water.")
                        .build())
                .build();
    ...
    public void run() {
        ...
        try {
            ...
            while (processSets) {
                ...
                try {
                    exec.runPy(exec.voltReaderSetup);
                    milliVolts = exec.getVoltage();
                    partsPerMil = exec.getTDSValue();
                } catch (Exception e) {
                    logger.error("Error reading data from sensor");
                    milliVolts = "Error reading mv";
                    partsPerMil = "Error reading ppm";
                }

                // TODO: Populate data block
                dataBlock.setDoubleValue(0, timestamp);
                dataBlock.setStringValue(1, milliVolts);
                dataBlock.setStringValue(2, partsPerMil);
